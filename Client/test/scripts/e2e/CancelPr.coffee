...
ConfirmCancelProcess =  element(`by`.buttonText('Cancel process'))
process
...
describe "2502", ->
  ...
  it "2500", ->
    ...
    expect(CancellCall.getText()).toBe('Please confirm that the Call Off process has to be canceled')
  ...
  it "2535", ->
    "Cancel Process from Calendar (with select date)"
    ...
    expect(Calendar).toBePresentAndDisplayed()
    expect(DateFrom.getText()).toBe('')
    expect(DateTo.getText()).toBe('')



  it "2506", ->
    "Cancel Process from Schedule (without select event)"
  ...
  it "2537", ->
    "Cancel Process from Schedule (with select event)"
  ...
  it "2542", ->
    "Cancel Process from Completed (without note)"
  ...
  it "2544", ->
    "Cancel Process from Completed (with note)"
