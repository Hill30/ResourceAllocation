...
describe "2388", ->
  ...
  it "2412", ->
    "User can input in form any letters"

    expect(SearchForm).toBePresentAndDisplayed()
    expect(SearchForm.getAttribute('value')).toBe('');
    SearchForm.sendKeys 'ivan'
    expect(SearchForm.getAttribute('value')).toBe('ivan');

  it "2410", ->
    "User can input in form any numerals"

    ClearSearchForm.click()
    expect(SearchForm.getAttribute('value')).toBe('');

    SearchForm.sendKeys '555'
    expect(SearchForm.getAttribute('value')).toBe('555');
  ...
  it "2444", ->
    ...
    expect(DateFirstP.getAttribute('class')).toBe('btn btn-default btn-sm active btn-info')

    expect(SelectedDate.getText()).toBe(checkF)
    ...
    expect(DateFrom.getText()).toBe(checkR)
    expect(DateTo.getText()).toBe(checkS)
  ...
  it "321", -> 
    ...
    addNewActivity = element(`by`.xpath('//button[@class="btn btn-default ng-binding"]'))
    expect(ptor.isElementPresent(addNewActivity)).toBe(true)
    addNewActivity.click()

    expect(employeeAdd.getAttribute('value')).toEqual("")
    expect(serviceDateAdd.getAttribute('value')).toEqual("")
    expect(startTimeAdd.getAttribute('value')).toEqual("")
    expect(endTimeAdd.getAttribute('value')).toEqual("")
    expect(serviceCodeAdd.getAttribute('value')).toEqual("?")
    expect(clientAdd.getAttribute('value')).toEqual("")
    expect(element(`by`.model('ar.duration')).getAttribute('value')).toBe('')

    
  it "321", -> 
    "Mandatory fields at create of record."

    expect(ptor.isElementPresent(clientAdd)).toBe(true)
    ptor.sleep 1000
    clientAdd.sendKeys 'm'
    ptor.sleep 1000
    element.all(`by`.repeater('match in matches')).get(0).click()  
    element(`by`.xpath('//a[@class="btn btn-default ng-binding"]')).click()
    ptor.sleep 1000

    parentSD = serviceDateAdd.element(`by`.xpath('ancestor::td'))
    #mandatorySD = parentSD.element(`by`.xpath('//div[@class="form-group"]/div[@class="alert-wrap"]'))
    mandatorySD = parentSD.element(`by`.css('div.form-group div.alert-wrap'))
    flagSD = parentSD.element(`by`.css('div.form-group div.required-flag'))
    expect(flagSD).toBePresentAndDisplayed()
    expect(ptor.isElementPresent(mandatorySD)).toBe(true)
    expect(mandatorySD).toNotHaveClass('ng-hide')

    serviceDateAdd.click()
    serviceDateAdd.sendKeys dateC 

    parentSD = serviceDateAdd.element(`by`.xpath('ancestor::td'))
    mandatorySD = parentSD.element(`by`.css('div.form-group div.alert-wrap'))
    expect(ptor.isElementPresent(mandatorySD)).toBe(true)
    expect(mandatorySD).toHaveClass('ng-hide')
